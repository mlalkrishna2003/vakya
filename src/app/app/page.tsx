        ...
        <div className="flex min-h-screen bg-[#030712] text-white">
            ...
            <aside className="w-80 border-r border-white/5 hidden lg:flex flex-col p-8 space-y-12 shrink-0 h-screen sticky top-0">
                ...
                <div className="space-y-4">
                    ...
                    <Link href="/" className="flex items-center gap-3 text-slate-500 hover:text-white transition-all px-4 w-full group py-2 text-sm font-bold">
                        ← Back to Vakya
                    </Link>
                    <button className="flex items-center gap-3 text-slate-500 hover:text-white transition-all px-4 w-full group py-2" aria-label="Sign Out">
            ...
            <main className="flex-1 p-6 md:p-12 overflow-y-auto">
                <header className="flex items-center justify-between mb-8 md:mb-16 max-w-7xl mx-auto">
                    ...
                    <div className="flex items-center gap-5">
                        ...
                        <div className="w-14 h-14 rounded-2xl glass border border-white/10 flex items-center justify-center text-teal-400 shadow-inner group cursor-pointer hover:border-teal-500 transition-all">
                ...
                <div className="max-w-7xl mx-auto min-h-[60vh]">
                    <AnimatePresence mode="wait">
                        {activeView === "overview" && (
                            <motion.div key="overview" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.4 }} className="grid grid-cols-1 xl:grid-cols-3 gap-12">
                                <div className="xl:col-span-2 space-y-12"><KnowledgeTrinity /></div>
                                <div className="space-y-6">
                                    <div className="glass p-8 border-teal-500/10 rounded-[2.5rem] sticky top-0 bg-[#030712]/50">
                                        ...
                                        <div className="aspect-square rounded-[2rem] bg-black/40 border border-white/5 flex flex-col items-center justify-center text-center p-8 space-y-6 relative overflow-hidden">
                                            ...
                                            <p className="italic text-slate-500 text-xs font-semibold leading-relaxed">Synchronizing with<br />Local Pulse Cluster...</p>
                                        </div>
                                    </div>
                                </div>
                            </motion.div>
                        )}
                        {activeView === "hub" && (<motion.div key="hub" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.4 }}><KnowledgeHub /></motion.div>)}
                        {activeView === "personas" && (<motion.div key="personas" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.4 }}><Personas /></motion.div>)}
                        {activeView === "vault" && (<motion.div key="vault" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.4 }}><Vault /></motion.div>)}
                        {activeView === "security" && (<motion.div key="security" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.4 }}><Security /></motion.div>)}
                        {activeView === "nexus" && (<motion.div key="nexus" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.4 }}><Nexus /></motion.div>)}
                        {activeView === "studio" && (<motion.div key="studio" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.4 }}><VoiceStudio /></motion.div>)}
                        {activeView === "elite" && (<motion.div key="elite" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.4 }}><EliteStudio /></motion.div>)}
                        {activeView === "acharya" && (<motion.div key="acharya" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.4 }}><Acharya /></motion.div>)}
                    </AnimatePresence>
                </div>
            </main>
            ...
            <nav className="fixed bottom-0 left-0 right-0 lg:hidden glass backdrop-blur-3xl border-t border-white/5 px-10 py-6 flex items-center justify-between z-50 rounded-t-[2.5rem]">
                <button onClick={() => setActiveView("overview")} aria-label="Overview" className={`transition-all duration-300 ${activeView === "overview" ? "text-teal-400 scale-125" : "text-slate-600 hover:text-white"}`}><LayoutDashboard className="w-7 h-7" /></button>
                <button onClick={() => setActiveView("hub")} aria-label="Knowledge Hub" className={`transition-all duration-300 ${activeView === "hub" ? "text-teal-400 scale-125" : "text-slate-600 hover:text-white"}`}><Sparkles className="w-7 h-7" /></button>
                <button onClick={() => setActiveView("vault")} aria-label="Vault" className={`transition-all duration-300 ${activeView === "vault" ? "text-teal-400 scale-125" : "text-slate-600 hover:text-white"}`}><Database className="w-7 h-7" /></button>
                <button onClick={() => setActiveView("personas")} aria-label="Personas" className={`transition-all duration-300 ${activeView === "personas" ? "text-teal-400 scale-125" : "text-slate-600 hover:text-white"}`}><Users className="w-7 h-7" /></button>
            </nav>
        </div>
    );
}
